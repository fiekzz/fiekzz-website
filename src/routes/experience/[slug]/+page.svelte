<script lang="ts">
	...
	let title = $derived(`${data.experience?.name ?? 'Not Found'} - Experiences`);
	let banner = $derived(
		data.experience?.logo?.mediaURL ?? Assets.Unknown.dark
	);

	let duration = $derived(
		`${getMonthAndYear(data.experience?.periodFrom)} - ${getMonthAndYear(data.experience?.periodTo ?? new Date())} · ${computeExactDuration(
			data.experience?.periodFrom ?? new Date(),
			data.experience?.periodTo ?? new Date()
		)}`
	);
</script>
	...
	{:else}
		<FancyBanner img={banner}>
			<div class="flex w-full flex-col items-center justify-center gap-4">
				...
				<Muted>{data.experience.company} · {data.experience.location} · {data.experience.type}</Muted>
				<Muted>{duration}</Muted>
				<Separator />
				<div class="flex flex-row flex-wrap justify-center gap-2">
					{#each data.experience.links as link (link.id)}
						<a href={link.link} target="_blank"><Badge variant="outline">{link.label}</Badge></a>
					{/each}
				</div>
				<div class="flex flex-row flex-wrap justify-center gap-2">
					{#each data.experience.skills as skill (skill.id)}
